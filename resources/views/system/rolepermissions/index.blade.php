        ...
        <tbody>
            @foreach($rolepermissions as $rolepermission)
                <tr>
                    ...
                    <td>
                        {{ $rolepermission->permission->display_name }}
                    </td>
                    <td>
{{--                        <a href="{{ URL::to('/system/userroles/'.$userrole->id.'/edit') }}" class="btn btn-success btn-sm pull-left">编辑</a> --}} 
                        {!! Form::open(array('url' => '/system/roles/' . $rolepermission->role->id . '/permissions/destroy/' . $rolepermission->permission->id, 'method' => 'delete', 'onsubmit' => 'return confirm("确定删除此记录?");')) !!}
