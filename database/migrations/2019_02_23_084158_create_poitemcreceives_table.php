...
class CreatePoitemcreceivesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('poitemcreceives', function (Blueprint $table) {
            $table->increments('id');

            $table->integer('poheadcreceive_id')->nullable();
            ...
            $table->smallInteger('status')->default(0);      // 0: 正常; -1: 数量原来未更新到pheadcs;

            $table->timestamps();

            $table->foreign('poheadcreceive_id')->references('id')->on('poheadcreceives')->onDelete('cascade');
        });
    }
